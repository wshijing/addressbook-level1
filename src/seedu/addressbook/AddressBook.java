    ...
    private static String executeCommand(String userInputString) {
        ...
        switch (commandType) {
            ...
            case COMMAND_SORT_WORD:
                ArrayList<String[]> toBeSortDisplayed = getAllPersonsInAddressBook();

                //toBeSortDisplayed.stream().map(String :: toLowerCase).collect(Collectors.toList());
                toBeSortDisplayed.sort(Comparator.comparing(index -> index[0]));
                showToUser(toBeSortDisplayed);
                return getMessageForPersonsDisplayedSummary(toBeSortDisplayed);
            default:
